...
```{r fig.cap="Visualization of Reinforcement Learning", fig.height=3, fig.width=5}
...
```{r network-illustration, fig.cap="Visualization of the work proposed by Mnih et al."}
knitr::include_graphics("fig-illustration.png")
```
...
```{r qvalue, fig.cap = "Visualization of the learned action-value function in the game Pong"}
...
```{r con-video, fig.cap= "But what is a convolution?"}
# Embedding video using package from https://ijlyttle.github.io/vembedr/
library("vembedr")
embed_url("https://www.youtube.com/watch?v=KuXjwB4LzSA")  %>%
  use_align("center")
```
...
```{r traincurve, fig.cap = "Traning curves tracking the agent's average score and average predicted action value"}
knitr::include_graphics("figure-traincurve.png")
```
...
```{r evaluation, fig.cap = "Comparison of the DQN agent with the best reinforcement learning methods in the literature"}
knitr::include_graphics("figure-evaluation.png")
```

The authors further evaluate the agent in all 49 games in the Atari 2600 platform and compare the results with the best reinforcement learning methods in the literature. Fig \@ref(fig:evaluation) summarizes the results of the comparison. The deep Q-network agent outperforms the best reinforcement learning methods and professional human game testers in majority of the games. It is worth to notice that the best performance games are those easier to play and easier for the agent to learn (e.g: Boxing, Breakout, Star gunner, Pinball, etc.). These games are short-horizon games, and the ultimate game strategy is to gain more points in the game. The Atari 2600 platforms are easy for the agent to learn, and the reward is simple and immediate. But for long-horizon games that doesn't give immediate reward, and may take a longer time to achieve a goal, the agent may not perform well in these cases. Montezuma's Revenge is one of the hardest games in the Atari 2600 platform, which require the player to control the character to explore the maze and collect the keys to open the doors (showed in Fig \@ref(fig:mont) [@image]). The reward will only be given when the player reach the end of the maze and open the door. The evaluation shows that the agent is basically play the game randomly (0% performance) and did not learn the strategy to play the game.

```{r mont, fig.cap = "Screenshot of the game Montezuma's Revenge"}
knitr::include_graphics("figure-mont.png")
```
...
```{r video, fig.cap= "DQN training for playing Atari's Breakout game"}
# Embedding video using package from https://ijlyttle.github.io/vembedr/
library("vembedr")
embed_url("https://youtu.be/TmPfTpjtdgg")  %>%
  use_align("center")
```
